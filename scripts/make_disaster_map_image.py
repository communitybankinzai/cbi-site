...
WEST, SOUTH, EAST, NORTH = 140.02, 35.74, 140.34, 35.92
ZOOM = 12
TILE = 256
...
def main(out_path):
    ...
    open_names = []
    seen = set()
    for s in (shelters.get('shelters') or []):
    ...
    header_h, footer_h = 74, 96
    out = Image.new('RGB', (size[0], size[1] + header_h + footer_h), (255, 255, 255))
    out.paste(canvas.convert('RGB'), (0, header_h))
    d = ImageDraw.Draw(out)

    d.rectangle([0, 0, size[0], header_h], fill=(22, 50, 79))
    d.text((20, 12), '印西市 災害状況整合MAP', font=load_font(28, True), fill=(255, 255, 255))
    d.text((20, 46), f'{stamp.strftime("%Y年%-m月%-d日 %H:%M")} 時点' if sys.platform != 'win32'
           else f'{stamp.year}年{stamp.month}月{stamp.day}日 {stamp.strftime("%H:%M")} 時点',
           font=load_font(18), fill=(200, 214, 230))
