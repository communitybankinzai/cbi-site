...
| 防災科研（NIED）リアルタイムリスクマップの重ね表示 | **見送り（許諾得られず）** | 2026-08-23に知財窓口へ問い合わせ→8/26「データ自体は非公開」との回答→当方から利用差し控え・無認証状態の共有・連携打診を返信→以後返信なし（当方の返信が最後）。**8/26の「データ自体は非公開」回答をもって実質断りと判断し 8/28 見送り確定**。配信URLへのアクセスは行わない。公式情報リンク（閲覧ページへの別窓リンク）のみ維持。経緯は保管庫 `cidao/2026-08-23_災害公式情報の情報源調査とタイムライン設計.md` |
