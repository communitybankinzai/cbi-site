...
1. **Instagram公開ハッシュタグ検索の接続**（Threadsは2026-08-17に検索専用アプリ方式で解決済み）。FacebookページとIGプロアカウントの連携が前提。手順は保管庫「SNS検索権限の設定手順」。
2. Blueskyの巡回失敗（HTML応答）の修正。
3. Threads検索がApp Review未承認でも公開投稿を取得できているかの実データ確認。
4. MAPのLocalStorage記録をSupabaseへ共有保存し、登録利用者・自主防災組織向けの認証、権限、承認、監査ログを実装する（下の3と同時に設計する。認証基盤を二度作らないため）。
5. SNS候補の確認・却下・採用状態をサーバー側へ保存し、複数端末で同期する。
6. 道路中心線データを使い、クリック点を道路へスナップする。必要なら交差点候補、区間編集、方向別規制を追加する。
7. 印西市公式の洪水浸水継続時間、家屋倒壊等氾濫想定区域を、利用条件と更新日を確認したうえで追加する（揺れやすさ・液状化は上表のとおり取り込めない）。
