...
- **⏱ タイムトライアル**（2026-08-20追加）: ☰メニューの「⏱ タイムトライアル」。設定は `TT_CONFIG`（参加要件=クイズ10問以上・正答率80%以上／年齢区分→レベル対応／コース=固定の文化財index列: 初級[44,39,0]・中級[43,3,6,44,39]・上級[3,6,43,41,0,44,13]／通過判定80m／事務局メール）に集約。流れ: 要件判定→年齢選択→ニックネーム→1か所目へテレポート＋カウントダウン6秒→onTickで計測・HUD表示（#ttHud）→全通過でttFinish→証明書canvas（タイム・正答率・検証コード=djb2ハッシュ）をPNG保存＋mailtoで事務局報告。自己ベストは localStorage `cbi-meta-tt-best-v1`。トライアル中は showEncounter を抑止。**注意: 計測はクライアント側のためタイム改ざんは技術的に可能（検証コードは簡易抑止）。厳密な競技運用にはサーバー記録が必要**
