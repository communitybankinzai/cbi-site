...
2. APIキー（index.html内のGoogle Maps APIキー）は出力・新規ファイルへ複製しない。リファラ制限・API制限は適切に設定済みと確認済み（2026-08-16）。**課金は実際に発生している：8月分 ¥316 を2026-09-01に決済済み**（「実測¥0・無料枠内」という以前の記載は誤り）。ただし**全額がメタバース由来ではない**：`creators-map` プロジェクトは公開前の3〜7月分も毎月 ¥98〜¥245 が課金されていた（入金レシート6通で確認）ため、**メタバース由来の増分は月¥151が上限**。SKU別の内訳は未確認。無料枠のリセットは**毎月1日 日本時間16:00**（太平洋時間0時／冬時間は17:00）。単価は $6.00/1,000イベント・無料枠1,000/月（SKU C6E1-98B2-DBD0）。経緯と確認手順は [[cidao/2026-09-01_MapTiles課金の実態と検証モード]]／[[cidao/2026-09-02_MapTiles課金の6か月履歴と切り分け]]
3. **開発中の動作確認は `?notiles=1`（検証モード）で行う。** 3Dワールドは1回開くたびに課金対象の root request が1回発生し、2026-09-01 は本番での繰り返し確認だけで9月予算の10%（約¥100）に達した。検証モードでは3D Tilesを読まないため課金がゼロで、ピン・メニュー・コントローラー操作・トンビはすべて動く（ローカルでも従来出せなかったピンが表示される）。街並みごと見たいときだけ通常URLを使う
...
- **費用ゼロ運用**（2026-08-21・**⚠ 効いているか未確認**）: Google側で 3D Tiles root requests per day=**30**（課金対象を無料枠1,000/月以下に構造的に抑える）設計。**ただし2026-09-01に月初1日で予算の10%（¥100）へ達しており、root上限が30のままかは要確認**（2026-08-21に有効化した割り当て自動調整がrootまで引き上げた疑い）。確認先は [割り当て画面](https://console.cloud.google.com/google/maps-apis/quotas?project=creators-map)（**you0810jmsdf@gmail.com でログインが必要**）。判定手順は [[cidao/2026-09-02_MapTiles課金の6か月履歴と切り分け]]。超過時は root.json が429→`showError` が「本日の3Dワールド利用枠に達しました」を表示（故障表示ではない）。renderer（タイル本体）は無償だが1日の回数上限があり、**Google既定の3万回では8/20・8/23に実際に枯渇したため 2026-08-23 に 500,000回/日 へ引き上げ済み**（Cloud Consoleで即時承認・無償のため費用は増えない。経緯は [[2026-08-23_MapTilesクォータ引き上げ申請]]）。実測1人あたり4,000〜8,000回なので**1日60人程度が目安**。**この値を変えたら CiDAO の `RENDERER_LIMIT_PER_DAY`・`site/admin/admin.js`・`site/metaverse/index.html` のフォールバック値も必ず合わせること**（警告の閾値がずれる）。イベント日は当日朝に **root** を一時引き上げる運用（運営方針ノート参照）。予算アラートは月1,000円・10/50/100%
