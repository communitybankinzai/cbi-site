...
実装は `musashiya.js`（別ファイル）＋文面 `musashiya-texts.json`（キー＝bunkazai.json の reportId、kids／adult の2種）。index.html 側は ☰「あそぶ」の `#musashiyaBtn`・`CLOSE_AFTER`・`applyMode` の表示リスト・`<script src="musashiya.js">` の4点だけ。武蔵屋（岩井家住宅主屋）→ 4km以内の19件から座標の重複を除いてランダムに3か所（武蔵屋から最近傍順）→ 武蔵屋ゴール。**タイムトライアルの仕組み（ttActive／ttCourse／ttPos／onTick の通過判定）をそのまま使い**、`ttServerStart`／`ttServerCheckpoint`（何もしない＝サーバーに記録しない）・`ttSpiritPass`（通過ポップアップ）・`ttShowCard`（次の目的地カード）・`ttFinish`（ゴールのポップアップを残す）・`ttAbort` を `msyState.on` の間だけ差し替える。**これらの関数名や onTick の順序（ttSpiritPass → ttPos++ → ttFinish）を変えるときは musashiya.js も直すこと**。会場では `?mode=event&event=musashiya` で、受付（CiDAO照合）後に開始画面が自動で出る。開始画面・ポップアップはコントローラー（○＝決定、←→＝こども／おとな）と Enter／Esc でも操作できる。検証は `?notiles=1` で `msyStart('kids')` → `ttCountdownEnd=0` → 目標へ `camera.setView` → `viewer.clock.onTick.raiseEvent(viewer.clock)` を4回（ペインが非表示でも通る）。**同日追記（版 b）**：開始位置は `goHome()`＝武蔵屋の南約50m（`HOME_BACK_DEG`）・地面＋14m（`sampleHeightMostDetailed`、取れなければ楕円体高60m）で北向き。`eventLook()` が白鳥（`setSwanVisible(true)`）と BGM（`bgmPreset="swanlake"`・`bgmCustom=null`。localStorage は書き換えない・`bgmUserOff` なら流さない）を設定。イベントURLでは自動再生の制限のため最初の pointerdown/keydown で流す。**版 c**：`ttShowCard` の差し替えはカードを出さない（開始直後に精霊の絵が大きく出たため）。`#ttHud` は `placeHud()` で `#topLeftBar` の下へ（`body.msyOn` の間）。こども用は `speakKids()` が `speechSynthesis` で読み上げ（声は ja の Keita→Ichiro→先頭。`speechText()` が「漢字（よみ）」を読みだけにする）。`?event=musashiya` では `showEncounter` を無効化。**版 d（モード分離）**：`MODE_LIST` に `musashiya` を追加し、モード選択画面にも「🏠 武蔵屋イベント」を置いた。`applyMode("musashiya")` は `body.modeMusashiya` を付け、`#musashiyaBtn`（`#topLeftBar` 直下・常時表示枠）だけを出し、`ttBtn`／`nightBtn`／`nightTourBtn`／`nightTtBtn`／`tbGroupPins`／`tbGroupPlay`／`panelToggle`／`puzzleBtn`／`tonbiToggleBtn`／`shopToggleBtn` を隠して `window.msyEnterMode()` を呼ぶ（別モードへ移ると `msyLeaveMode()`＝走行中なら中止）。musashiya.js は `inEventMode()`（body クラス、または旧 `?event=musashiya`）で判定する。**会場URLは `?mode=musashiya`**。**版 f**：開始は `start()`→`state.waiting`（`ttActive=false`・HUDに「○でスタート」）→ `readyGo()`（○／Enter）で `ttActive=true`・3.5秒カウントダウン。開始位置は地面＋3m（`HOME_CLEARANCE_M`）、index.html の `keepAboveGround` は `window.flightClearanceM`（モード中 3）を見る。地面が取れないときは楕円体高45mに置き、`settleToGround()` が取れ次第降ろす。`startAltTimer()` が `#ttHud` に `#msyAlt`（地上からの高さ）を足す。`placeSwanBtn()` が `#swanToggleBtn` を常時表示列へ移す（離脱時に戻す）。声は `state.voice`（male/female・`VOICE_NAMES`）、`speak()` が `bgmDuck()` で BGM を 0.08 に下げ、終了で戻す。**版 h（コース変更）**：スタート `SWAN_HOME`（本埜の白鳥の郷・国土地理院「印西市笠神2373」の代表点・**要現地確認**）→ 経由 `station()`（`SPOTS[0]` 千葉NT中央駅・`STATION_RADIUS_M`=150）→ 文化財3か所（駅から最近傍順）→ 武蔵屋。第1区間は駅が BUNKAZAI に無いため `leg0Tick()`（100ms）が自前で計測・HUD更新し、通過時に `ttStartMs` を引き継いで `ttActive=true`（index.html の onTick へ）。声は `#msyVoiceSel`（`state.voiceName`・localStorage `cbi-meta-msy-voicename-v1`）が男女の自動選択より優先。`speak()` は cancel の150ms後に話し、`state.utter` に参照を保持。`rtDetach(true)` がモード中は Presence を切って光点・同行者リストを消す（離脱で `rtConnect()`）。**版 i〜k**：`tilesReady()`（`tileset.tilesLoaded`）まで `canStart()` が false（`TILES_WAIT_MAX_MS`=45秒で解除）、`groundOk()`（楕円体高 -50〜400m）を通った地面だけ使う。**flight-ground.js** の enforce は ground が -200〜1000m のときだけ押し上げる（読み込み前の数千mで「高さ7777m」になっていた）。左下の高度表示も同じ範囲。ゴールで `speak()`（両モード）。`okLabel()`＝`padKind()`（gamepad.id で ps／xbox 判定）でボタン名を切替、決定は buttons[0]／[1] のどちらでも。**race-radar.js（新規）**：`window.SkyTag.render／clear／reset` を包み、タイムレース（`race.mode !== "tag"`）中に各画面へ相手の機体（`p.rrEntity`）とレーダー（`p.rrScope`・相手＋チェックポイント）を描く。vs-race.js／sky-tag.js は無変更。ヘッドレス（ペイン非表示）では rAF が止まり frameLoop が回らないので、検証は `SkyTag.render(vsRacePlayers, vsRaceState)` を直接呼ぶ。**版 l・m**：`speak()` は `onstart` を 2.5 秒待ち（`state.startWatch`）、始まらなければ `localService` の同性の声で読み直す（`state.lastSpeech.fallback`）。`imgFor()` は kids＝精霊カード優先・adult＝写真優先。**版 n**：`imgFor()` は adult で写真なしのとき `{placeholder:true, lines:[…]}` を返し、`imgHtml()` が `.msyNoPhoto`（破線枠）に「利用できる写真がありません。写真提供をお待ちしています。」を出す。**版 o**：開始高度は `HOME_CLEARANCE_M`=500（地上）・`HOME_FALLBACK_H`=540・`START_PITCH_DEG`=-14。白鳥の郷の座標は 2026-09-13 に中司さんが本番で確認済み。index.html の操作説明は `PAD_LABELS`／`padKindForHelp()`／`helpGamepadHtml(kind)` でコントローラー種別に応じて生成（`updateHelpBox` のモードキーに種別を含む）。**版 p（録音ナレーション）**：`assets/narration/{male,female}/<reportId|station|goal>_<kids|adult>.mp3`（edge-tts・`build_narration.py`・`manifest.json`）。`narrate(key, fallbackText)` が `useRec()`（`state.voiceName==="__rec"`・既定）なら `Audio` で再生し、失敗時は `speak()`。`preloadNarration()` が開始時にコースぶんを先読み。**musashiya-texts.json の文を変えたら `python assets/narration/build_narration.py` で作り直すこと**（駅・ゴールの定型文はスクリプト内にもあり、musashiya.js と揃える）。**版 q〜s（VOICEVOX・設定パネル）**：`#msyVoiceBtn`（`placeVoiceBtn()` が `#tbGroupSettings .tbMenu` 先頭へ・モード中のみ）→ `#msyVoicePanel`（`renderVoicePanel()`）。`VOICE_KEY` は v2（v1 の保存値は無視）。`VOICE_INFO`（zundamon＝既定／male／female）。`assets/narration/zundamon/` は `build_narration_voicevox.py`（エンジン `C:/Tools/voicevox_engine/windows-cpu/run.exe` を起動してから実行・1ファイル約1分）。クレジット「VOICEVOX:ずんだもん」は開始画面の `.msyVer` と README に記載（削らない）。**2Pレース（vs-race.js）**：`buildRaceCourses()` の先頭に固定コース `musashiya_vs`（start＝白鳥の郷・高さ300／CP＝NT中央駅→BUNKAZAI[5]木下貝層→[42]上宿古墳→[17]馬込遺跡出土瓦塔→[49]岩井家住宅主屋）。開始ボタンのハンドラが `body.modeMusashiya` を**applyMode("event") より前に**判定して `enableVsRace("cultural-vs","musashiya_vs")`・`race.returnMode="musashiya"`、終了時（URL の race/course を消す箇所）で `applyMode("musashiya")` に戻す。加速は buttons[2]（Xbox の X／PS の □）押しっぱなしで×4、LB/RB（buttons[4]/[5]）はロール。**2Pの3か所ランダム化**：`musashiya_vs` に `pick()`（4km圏・150m以内は同じ場所扱い・駅から最近傍順）。`resolveCourse()` の fixed 分岐は `raw.pick` があれば呼ぶ。`resetRace(true)`（RESET・レース後の○/B）で pick のあるコースは `setCourse()` をやり直して選び直す（中は `resetRace(false)` なので再帰しない）。1人用 `pickCourse()` の重複判定も座標一致→150m以内に変更。**版 w（年齢別の声）**：`state.voices = {kids, adult}`（localStorage `cbi-meta-msy-voice-v2-kids`／`-adult`）、`AGE_VOICES`（kids: zundamon/male/female、adult: male/female・先頭が標準）。`state.voice` は `Object.defineProperty` で「いまの年齢の声」に振り分ける getter/setter。**state の初期化時に const を参照しないこと**（宣言前参照で失敗し、以前は try/catch に隠れて保存値が読めていなかった）。到着・駅の読み上げはこども・おとなとも。説明文は2026-09-13時点で**下書き（中司さん確認前）**。武蔵屋自身の写真は無く精霊カードで代用中（9/19の現地確認で岩井さんの許可を得て撮影予定）。
