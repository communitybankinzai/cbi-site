...
2. APIキー（index.html内のGoogle Maps APIキー）は出力・新規ファイルへ複製しない。リファラ制限・API制限は適切に設定済みと確認済み（2026-08-16）。**課金は実際に発生している：8月分 ¥316 を2026-09-01に決済済み**（「実測¥0・無料枠内」という以前の記載は誤り）。無料枠のリセットは**毎月1日 日本時間16:00**（太平洋時間0時／冬時間は17:00）。SKU別の内訳は未確認。経緯と確認手順は [[cidao/2026-09-01_MapTiles課金の実態と検証モード]]
