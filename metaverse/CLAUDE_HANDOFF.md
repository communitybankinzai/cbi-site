...
- **写真・詳細モーダル・レベル別クイズ・認定証**（2026-08-19追加）:
  ...
  - 文化財ピンクリック→詳細モーダル（写真＋出典・解説＋出典・この文化財のクイズ3問=初級/中級/上級・公式リンク・▶現地へ行く）。ずかん詳細の「📷 写真とクイズを見る」からも開く
  - クイズタブはコース制: 🔰初級（区分/種別/写真あて）・🥈中級（時代/場所あて）・🥇上級（`quiz`フィールド=市公式記載事実の手作り50問）・🎲おまかせ（旧全体クイズ、ユーザー指示で存置）。毎回ランダム生成、答え合わせに出典リンク表示
